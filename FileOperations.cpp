...
FileOperations::FileOperations(TextEditor* editor)
    : editor_(editor)
{
}
...
void FileOperations::openFile(const std::string& fname)
{
    ...
    if (!file.is_open())
    {
        editor_->addOutput(editor_->icons["error"], "Could not open file: " + fname + " (" + std::strerror(errno) + ")");
        return;
    }
    ...
    if (editor_->content.size() == 0)
        ...
        editor_->addOutput(editor_->icons["folder"], "Opened: " + fname + " (" + std::to_string(editor_->content.size()) + " bytes)");
}
...
void FileOperations::saveFile()
{
    ...
    {
        editor_->addOutput(editor_->icons["error"], "Could not save file: " + editor_->filename);
    }
}
...
void FileOperations::showOpenDialog()
{
    ...
    {
        editor_->addOutput(editor_->icons["error"], std::string("NFD Error: ") + NFD_GetError());
    }
}
