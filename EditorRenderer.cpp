...
void EditorRenderer::renderGrid(ImDrawList *drawList, ImVec2 pos, float viewW,
                                float viewH, float cellWidth, float lineHeight,
  ...
  for (float y = startY; y < pos.y + viewH; y += gridSpacingY) {
    ...
    drawList->AddLine(ImVec2(pos.x, yMid), ImVec2(pos.x + viewW, yMid),
                      gridColor);
  }
}

static float computeGutterWidth(int totalLines, float cellWidth) {
...
void EditorRenderer::renderSelection(ImDrawList *drawList, ImVec2 pos,
  ...
  for (int line = selMinLine;
       line <= selMaxLine && line < (int)editor_->lineCache.size(); line++) {
    if (line < firstVisibleLine || line >= lastVisibleLine)
      continue;

    int colStart = (line == selMinLine) ? selMinCol : 0;
    int colEnd = (line == selMaxLine)
                     ? selMaxCol
                     : (int)editor_->lineCache[line].text.size();

    float lineY = pos.y + padY - editor_->scrollY +
                  (line - firstVisibleLine) * lineHeight +
                  firstVisibleLine * lineHeight;
    float x1 = pos.x + padX - editor_->scrollX + colStart * cellWidth;
    float x2 = pos.x + padX - editor_->scrollX + colEnd * cellWidth;

    drawList->AddRectFilled(ImVec2(x1, lineY), ImVec2(x2, lineY + lineHeight),
                            selectionColor);
  }
}

void EditorRenderer::renderVisibleLines(ImDrawList *drawList, ImVec2 pos,
                                        float padX, float padY,
                                        int firstVisibleLine,
                                        int lastVisibleLine) {
  editor_->maxContentWidth = 0.0f;
  float y = pos.y + padY -
            (editor_->scrollY - firstVisibleLine * editor_->lineHeight);

  for (int i = firstVisibleLine;
       i < lastVisibleLine && i < (int)editor_->lineCache.size(); i++) {
...
void EditorRenderer::handleMouseInput(ImVec2 pos, float padX, float padY,
                                      float cellWidth, float lineHeight) {
  ...
  if (ImGui::IsItemClicked()) {
    ...
    if (editor_->lineCache.empty()) {
      ...
      clickedLine =
          std::clamp(clickedLine, 0, (int)editor_->lineCache.size() - 1);

      int clickedCol = (int)(localX / cellWidth + 0.5f);
      clickedCol = std::clamp(clickedCol, 0,
                              (int)editor_->lineCache[clickedLine].text.size());
  ...
  if (editor_->isDragging && ImGui::IsMouseDown(ImGuiMouseButton_Left)) {
    if (!editor_->lineCache.empty()) {
      ...
      dragCol =
          std::clamp(dragCol, 0, (int)editor_->lineCache[dragLine].text.size());
...
void EditorRenderer::handleKeyboardInput() {
  ...
  if (ImGui::IsKeyPressed(ImGuiKey_LeftArrow)) {
    if (io.KeyShift) {
      ...
      if (editor_->hasSelection()) {
        editor_->cursorIndex =
            std::min(editor_->selectionStart, editor_->selectionEnd);
        editor_->selectionStart = editor_->selectionEnd = -1;
      } else if (editor_->cursorIndex > 0) {
        editor_->cursorIndex--;
      }
  ...
  if (ImGui::IsKeyPressed(ImGuiKey_RightArrow)) {
    if (io.KeyShift) {
      ...
      if (editor_->hasSelection()) {
        editor_->cursorIndex =
            std::max(editor_->selectionStart, editor_->selectionEnd);
        editor_->selectionStart = editor_->selectionEnd = -1;
      } else if (editor_->cursorIndex < (int)editor_->content.size()) {
        editor_->cursorIndex++;
      }
  ...
  if (ImGui::IsKeyPressed(ImGuiKey_End)) {
    ...
    if (line < (int)editor_->lineCache.size()) {
      if (io.KeyShift && editor_->selectionStart == -1)
        editor_->selectionStart = editor_->cursorIndex;
      editor_->cursorIndex = editor_->lineColToIndex(
          line, (int)editor_->lineCache[line].text.size());
      if (io.KeyShift)
        editor_->selectionEnd = editor_->cursorIndex;
      else
        editor_->selectionStart = editor_->selectionEnd = -1;
...
void EditorRenderer::renderCaret(ImDrawList *drawList, ImVec2 pos, float padX,
                                 float padY, float cellWidth, float lineHeight,
                                 bool isFocused) {
...
void EditorRenderer::handleEditorInput(ImVec2 pos, float viewW, float viewH,
                                       float padX, float padY, float cellWidth,
                                       float lineHeight, bool isFocused) {
...
void EditorRenderer::renderHorizontalScrollbar(ImVec2 pos, float viewW,
                                               float viewH, float scrollbarH,
                                               float visibleW, float trackW,
                                               float extraPad) {
  ...
  dl->AddRectFilled(hBarPos, ImVec2(hBarPos.x + trackW, hBarPos.y + scrollbarH),
                    bgCol);
  ...
  float thumbX =
      (denom > 0.0f) ? (editor_->scrollX / denom) * trackRange : 0.0f;
  ...
  if (editor_->hDragging) {
    if (ImGui::IsMouseDown(ImGuiMouseButton_Left)) {
      ...
      editor_->scrollX =
          (denom > 0.0f) ? (newThumbX / trackRange) * denom : 0.0f;
    } else {
  ...
  if (ImGui::IsItemClicked()) {
    ...
    if (mouseX < thumbX)
      ...
      float maxScrollX =
          std::max(0.0f, (editor_->maxContentWidth + extraPad) - visibleW);
      editor_->scrollX = std::min(maxScrollX, editor_->scrollX + page);
    }
  }

  ImU32 thumbCol =
      editor_->hDragging ? fillAct : (thumbHovered ? fillHot : fillCol);
  dl->AddRectFilled(thumbMin, thumbMax, thumbCol, 3.0f);
}

void EditorRenderer::renderVerticalScrollbar(ImVec2 pos, float viewW,
                                             float viewH, float scrollbarW,
                                             float visibleH, float trackH,
  ...
  ImDrawList *dl = ImGui::GetWindowDrawList();
  dl->AddRectFilled(vBarPos, ImVec2(vBarPos.x + scrollbarW, vBarPos.y + trackH),
                    bgCol);
  ...
  float thumbY =
      (vDenom > 0.0f) ? (editor_->scrollY / vDenom) * vTrackRange : 0.0f;

  ImVec2 vThumbMin = ImVec2(vBarPos.x, vBarPos.y + thumbY);
  ImVec2 vThumbMax =
      ImVec2(vBarPos.x + scrollbarW, vBarPos.y + thumbY + thumbH);
  ...
  if (editor_->vDragging) {
    if (ImGui::IsMouseDown(ImGuiMouseButton_Left)) {
      ...
      editor_->scrollY =
          (vDenom > 0.0f) ? (newThumbY / vTrackRange) * vDenom : 0.0f;
    } else {
  ...
  ImU32 vThumbCol =
      editor_->vDragging ? fillAct : (vThumbHovered ? fillHot : fillCol);
  dl->AddRectFilled(vThumbMin, vThumbMax, vThumbCol, 3.0f);
}

void EditorRenderer::renderScrollbars(ImVec2 pos, float viewW, float viewH,
                                      float scrollbarW, float scrollbarH,
                                      float cellWidth, float lineHeight,
                                      float visibleW, float visibleH) {
  float extraPad = cellWidth;
  float totalContentHeight = editor_->lineCache.size() * lineHeight;

  renderHorizontalScrollbar(pos, viewW, viewH, scrollbarH, visibleW, viewW,
                            extraPad);
  renderVerticalScrollbar(pos, viewW, viewH, scrollbarW, visibleH, viewH,
                          totalContentHeight);
...
void EditorRenderer::renderEditor() {
  if (ImGui::Begin("Editor", nullptr,
    ...
    int firstVisibleLine =
        std::max(0, (int)std::floor(editor_->scrollY / editor_->lineHeight));
    int lastVisibleLine = std::min(
        (int)editor_->lineCache.size(),
        firstVisibleLine + (int)std::ceil(viewH / editor_->lineHeight) + 1);

    // gutter for line numbers
    // compute gutterWidth (does NOT include the external padX)
    float gutterWidth =
        editor_->showLineNumbers
            ? computeGutterWidth((int)editor_->lineCache.size(), cellWidth)
            : 0.0f;
    // textPadX shifts the text area: gutter occupies gutterWidth, then we apply
    // padX between gutter and text
    ...
    if (editor_->showLineNumbers) {
      ...
      drawList->AddRectFilled(
          ImVec2(roundf(gutterStart.x), roundf(gutterStart.y)),
          ImVec2(roundf(gutterEnd.x), roundf(gutterEnd.y)), gutterBg);

      // compute digits reserved (must match computeGutterWidth logic: reserve
      // at least 6)
      ...
      // inner area where numbers live: start + sidePad, width =
      // digits*cellWidth
      ...
      for (int line = firstVisibleLine; line < lastVisibleLine; ++line) {
        ...
        // horizontally center the number inside the reserved digits area and
        // round
        ...
        float lineTop =
            pos.y + padY + (line - firstVisibleLine) * editor_->lineHeight -
            (editor_->scrollY - firstVisibleLine * editor_->lineHeight);
        float textLineHeight = ImGui::GetTextLineHeight();
        float numY = lineTop + (editor_->lineHeight - textLineHeight) * 0.5f;
        numY = roundf(numY);

        drawList->AddText(ImVec2(numX, numY),
                          ImGui::GetColorU32(ImGuiCol_TextDisabled), buf);
      }
    }

    // Grid
    renderGrid(drawList, pos, viewW, viewH, cellWidth, editor_->lineHeight,
               textPadX, padY);
    ...
    // render selection, lines, caret, and handle input — pass textPadX so
    // rendering is shifted
    renderSelection(drawList, pos, cellWidth, editor_->lineHeight, textPadX,
                    padY, firstVisibleLine, lastVisibleLine);
    renderVisibleLines(drawList, pos, textPadX, padY, firstVisibleLine,
                       lastVisibleLine);
    renderCaret(drawList, pos, textPadX, padY, cellWidth, editor_->lineHeight,
                isFocused);

    handleEditorInput(pos, viewW, viewH, textPadX, padY, cellWidth,
                      editor_->lineHeight, isFocused);

    // scrollbars (leave cellWidth so they can compute extraPad)
    renderScrollbars(pos, viewW, viewH, scrollbarW, scrollbarH, cellWidth,
                     editor_->lineHeight, viewW, viewH);
  }
  ImGui::End();
}
