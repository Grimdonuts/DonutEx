...
IconManager::IconManager(TextEditor *editor) : editor_(editor) {}

ImTextureID IconManager::loadSVGTexture(const char *filename,
                                        float targetHeight) {
  NSVGimage *image = nsvgParseFromFile(filename, "px", 96);
  if (!image) {
    editor_->addOutput(editor_->icons["error"],
                       std::string("Could not open SVG: ") + filename);
...
void IconManager::loadIcons(float dpiScale) {
  ...
  editor_->icons["document"] =
      loadSVGTexture("icons/document.svg", targetIconSize);
  editor_->icons["settings"] =
      loadSVGTexture("icons/settings.svg", targetIconSize);
  ...
  editor_->icons["refresh"] =
      loadSVGTexture("icons/refresh.svg", targetIconSize);
  editor_->icons["checkmark"] =
      loadSVGTexture("icons/checkmark.svg", targetIconSize);
}

bool IconManager::iconTextButton(const char *id, ImTextureID icon,
                                 const char *label, const ImVec2 &buttonSize) {
