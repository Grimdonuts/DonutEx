...
void TextEditor::showSaveDialog(const std::string &defaultFileName)
{
    ...
    nfdresult_t result = NFD_SaveDialog(&savePath, filters, 4, nullptr, defaultFileName.c_str());
    if (result == NFD_OKAY)
    {
        filename = savePath;
        saveFile();
...
if (showGrid)
{
    ...
    {
        float yMid = roundf(y + gridSpacingY * 0.75f); // tweak this 0.75f if lines seem too high/low
        draw_list->AddLine(ImVec2(pos.x, yMid),
                           ImVec2(pos.x + viewW, yMid), gridColor);
    }
}
...
float caretBottom = caretTop + lineHeight * 0.75f;   // slightly shorter caret
draw_list->AddLine(ImVec2(caretScreenX, caretTop),
        ...
        dl->AddRectFilled(hBarPos, ImVec2(hBarPos.x + hBarSize.x, hBarPos.y + hBarSize.y), bgCol);
        ...
        dl->AddRectFilled(vBarPos, ImVec2(vBarPos.x + vBarSize.x, vBarPos.y + vBarSize.y), bgCol);
