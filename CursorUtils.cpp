...
CursorUtils::CursorUtils(TextEditor *editor) : editor_(editor) {}
...
ImVec2 CursorUtils::getCursorScreenPos() {
  ...
  ImVec2 innerOrigin(editor_->editorRectMin.x + style.FramePadding.x,
  ...
  float caretY =
      (float)editor_->cursorLine * editor_->lineHeight - state->Scroll.y;
  ImVec2 caretScreen(innerOrigin.x + caretX, innerOrigin.y + caretY);
