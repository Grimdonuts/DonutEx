...
private:
  ...
  void renderVisibleLines(ImDrawList *drawList, ImVec2 pos, float padX,
                          float padY, int firstVisibleLine,
                          int lastVisibleLine);
  void handleEditorInput(ImVec2 pos, float viewW, float viewH, float padX,
                         float padY, float cellWidth, float lineHeight,
                         bool isFocused);
  void handleMouseInput(ImVec2 pos, float padX, float padY, float cellWidth,
                        float lineHeight);
  ...
  void renderHorizontalScrollbar(ImVec2 pos, float viewW, float viewH,
                                 float scrollbarH, float visibleW, float trackW,
                                 float extraPad);
  void renderVerticalScrollbar(ImVec2 pos, float viewW, float viewH,
                               float scrollbarW, float visibleH, float trackH,
